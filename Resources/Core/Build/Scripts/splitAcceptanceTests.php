...
/**
 * This script is typically executed by bamboo.
 *
 * The script expects to be run from the core root:
 * ./vendor/typo3/testing-framework/core/Build/Scripts/splitAcceptanceTests.php <numberOfChunks>
 *
 * Verbose output with 8 chunks:
 * ./vendor/typo3/testing-framework/core/Build/Scripts/splitAcceptanceTests.php 8 -v
 *
 * It's purpose is to find all core Backend acceptance tests and split them into
 * pieces. On bamboo, there are for example 8 agents and each picks one
 * chunk of tests. This way, acceptance tests are run in parallel on
 * multiple agents and thus reduce the overall runtime of the test suite.
 *
...
class SplitAcceptanceTests extends NodeVisitorAbstract
{
    ...
    public function execute()
    {
        ...
        foreach ($testStats as $testFile => $numberOfTestsInFile) {
            ...
            $content = str_replace('../../../../../../typo3/sysext/core/Tests/', '', $testFile) . "\n";
